...
const CorrectionComponent = ({ correction, diaryThumbnail }: Props) => {
  ...
  const normalized = (() => {
    ...
    const diaryTitle: string =
      c?.diaryTitle ?? c?.diaryInfo?.diaryTitle ?? c.title ??
      pickDeep(c, [
    ...
    const diaryId: number | string | undefined =
      c?.diaryId ?? c?.diaryInfo?.diaryId ?? c.postId ?? c.entryId ??
      pickDeep(c, [
    ...
    const diaryThumbnailValue: string | undefined =
      diaryThumbnail ??
      c?.diaryThumbnailUrl ?? c?.diaryInfo?.thumbImg ??
    ...
    const rawProfile = c.member ?? c.memberProfile ?? c.receiver ?? c.writer ?? c.owner ?? c.author ?? c.user ?? {};
    const member = {
        memberId: rawProfile.memberId ?? rawProfile.id ?? 0,
        username: rawProfile.username ?? '',
        nickname: rawProfile.nickname ?? '',
        profileImageUrl: rawProfile.profileImageUrl ?? rawProfile.profileImage ?? '',
    };
    ...
    return {
      ...
      diaryThumbnail: diaryThumbnailValue,
    };
  })();
  ...
  const isSavedList = !!savedId && savedId > 0;
  ...
  const onClickLike = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      const nextCount = typeof res?.likeCount === "number" ? res.likeCount : nextLiked ? likeCount + 1 : Math.max(0, likeCount - 1);
      setLiked(nextLiked);
  ...
  const comments = useMemo(() => {
    const contents = (listData?.result as any)?.contents ?? [];
  ...
  useEffect(() => {
    if (openReply || ((correction as any).commentCount ?? 0) > 0) {
      return;
    }
    if (((correction as any).commentCount ?? 0) === 0) {
      fetchComments({ correctionId: correctionId, page: 1, size: 1 });
    }
  }, [correctionId, (correction as any).commentCount, openReply, fetchComments]);
  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
      ...
      <div className="mt-4 flex items-center justify-end gap-6 text-sm text-gray-500">
        <button onClick={toggleReply} className="flex items-center gap-1 rounded px-1.5 py-1 hover:bg-gray-100 cursor-pointer">
          <img src={openReply ? "/images/commentIcon.svg" : "/images/emptycommentIcon.svg"} alt="댓글" className="w-5 h-5"/>
          <span>{commentCount}</span>
        </button>
        <button onClick={onClickLike} disabled={liking} className="flex items-center gap-1 rounded px-1.5 py-1 hover:bg-gray-100 disabled:opacity-60 cursor-pointer" aria-pressed={liked}>
          <img src={liked ? "/images/HeartIcon.svg" : "/images/EmptyHeartIcon.svg"} alt="좋아요" className={`w-5 h-5 ${liked ? "scale-110" : ""}`}/>
      ...
      {openReply && (
        <div className="mt-3 space-y-3">
          ...
          {!listLoading && comments.length === 0 && ( <div className="text-sm text-gray-400">{t.NoComments ?? "댓글이 없습니다."}</div> )}
          {!listLoading &&
            comments.map((c: {
              commentId: number;
              content: string;
              ...
              <div key={c.commentId} className="border-t border-gray-200 pt-3">
                <div className="flex items-center gap-2 text-sm text-gray-600">
                  <img src={c.member?.profileImageUrl ?? "/images/profileimage.svg"} className="h-6 w-6 rounded-full bg-gray-200" alt="프로필"/>
      ...
      <div className="mt-4 flex cursor-pointer items-center gap-3" onClick={handleNavigate}>
        ...
        <div className="text-body1 text-gray-700">
          {(normalized.diaryId ?? correctionId) != null && ( <span className="mr-2 text-gray-500">#{normalized.diaryId ?? correctionId}</span> )}
      ...
      {deleteLikeOpen && (
        <AlertModal
          ...
          onClose={(e) => { e.stopPropagation(); setDeleteLikeOpen(false); }}
