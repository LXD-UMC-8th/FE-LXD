...
const CommonComponentInDiaryNFeed = (_props: CommonComponentInDiaryNFeedProps) => {
  ...
  return (
    <div className="relative w-full bg-white rounded-2xl shadow px-6 py-5 space-y-4">
        ...
        <div>
          {isFeedTab ? (
            <div className="flex items-center gap-3">
              ...
              <span className="text-xs text-gray-400 ml-2 mt-0.5">2025.06.16 오후 02:44</span>
            </div>
          ) : (
            isMyDiaryTab && (
              <div className="text-sm text-gray-500 font-medium">#3 · 2025.07.09</div>
            )
          )}
        </div>
            ...
            <>
              ...
              {menuOpen && (
                <div className="absolute top-8 right-0 bg-white border rounded-md shadow-lg w-28 z-50">
                  <button className="w-full px-4 py-2 text-sm hover:bg-gray-100 text-left">수정하기</button>
                  <button className="w-full px-4 py-2 text-sm text-red-500 hover:bg-gray-100 text-left">삭제하기</button>
                </div>
              )}
            </>
          )}
      ...
      <p className="text-sm text-gray-700 leading-relaxed whitespace-pre-line">
        요즘 10시쯤 자고 6시쯤 일어나는 루틴을 유지하려고 하는 중. 아직 완벽하진 않은데, 일찍 자려는 의식이 생긴 것만으로도 괜찮은 변화 같음...
      </p>
